		...
		jQuery.fn.dateSelectBoxes = function(monthElem, dayElem, yearElem, keepLabels)
			{
				...
				function updateDays() {
					...
					var month = parseInt(monthElem.val() == "" ? $(monthElem).find("option[selected]").val() : monthElem.val(), 10);
					if (!month) {
                    ...
                    if(parseInt(selected[0], 10) > days.length - 1){
                        selected = ["1"];
                    }
                    ...
                    //need for pretty view
                    $(dayElem).trigger("refresh");
                }
                $(document.body).on("change", yearElem,function() {
					updateDays();
				});
				$(document.body).on("change",monthElem,function(){
					updateDays();
				});
			};
}(jQuery));